...
type Client struct {
	...
	CacheTTL         int
	SkipVersionCheck bool // Skip server version check on initialization
}

// NewClient returns a new PowerDNS client
func NewClient(serverURL string, apiKey string, configTLS *tls.Config, cacheEnable bool, cacheSizeMB string, cacheTTL int, skipVersionCheck bool) (*Client, error) {
