	...
	return &schema.Provider{
		Schema: map[string]*schema.Schema{
			...
			"skip_version_check": {
				Type:        schema.TypeBool,
				Optional:    true,
				DefaultFunc: schema.EnvDefaultFunc("PDNS_SKIP_VERSION_CHECK", false),
				Description: "Skip the version check when initializing the provider. Allows the provider to be configured with a dummy or unreachable server URL.",
			},
		},
